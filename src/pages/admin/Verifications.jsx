...
const Verifications = () => {
  ...
  const campuses = Array.isArray(campusesData) ? campusesData :
                   campusesData?.data && Array.isArray(campusesData.data) ? campusesData.data :
                   campusesData?.items && Array.isArray(campusesData.items) ? campusesData.items : [];
    ...
    onError: (error) => {
      toast.custom(() => (
        <div className="bg-white rounded-lg p-3 text-sm border-2 border-red-500 shadow-lg max-w-sm w-full break-words">
          Failed to process verification: {error.response?.data?.message || error.message}
        </div>
      ));
    }
  });
  ...
  const bulkReviewMutation = useMutation({
    mutationFn: ({ verificationIds, status, review_note }) =>
      adminService.bulkReviewVerifications(verificationIds, status, review_note),
    onSuccess: (data, variables) => {
      const actionText = variables.status === 'approved' ? 'approved' : 'rejected';
      toast.custom(() => (
        <div className="bg-white rounded-lg p-3 text-sm border-2 border-green-500 shadow-lg max-w-sm w-full break-words">
          {variables.verificationIds.length} verifications {actionText} successfully!
        </div>
      ));
      queryClient.invalidateQueries(['admin-verifications']);
      queryClient.invalidateQueries(['verification-stats']);
  ...
  const handleSingleReview = (verificationId, action) => {
    const review_note = action === 'reject' ? prompt('Please provide a reason for rejection:') : '';
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Review and approve student verification requests</p>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <select
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, campus_id: e.target.value }))}
            className="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
          >
          ...
          <button
            onClick={() => setFilters({ status: 'pending', search: '', campus_id: '' })}
      ...
      <div className="bg-white rounded-xl border border-gray-200">
              ...
              <tr>
                <th className="px-6 py-3 text-left">
                  <input
                    ...
                    checked={selectedVerifications.length === verifications?.items?.length && verifications?.items?.length > 0}
                  ...
                  <tr key={verification.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex items-center gap-1">
                        <Calendar size={14} />
                        {verification.submitted_at ? formatDate(verification.submitted_at) : 'N/A'}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex items-center gap-2">
                        ...
                        {verification.status === 'pending' && (
                          <>
                            <button
                              onClick={() => handleSingleReview(verification.id, 'approve')}
                            ...
                            <button
                              onClick={() => handleSingleReview(verification.id, 'reject')}
                              disabled={reviewMutation.isPending}
        ...
        {verifications?.total > 10 && (
          <div className="px-6 py-3 border-t border-gray-200 flex items-center justify-between">
            <div className="text-sm text-gray-500">
              Showing {(verifications.offset || 0) + 1} to {Math.min((verifications.offset || 0) + (verifications.limit || 10), verifications.total)} of {verifications.total} results
            </div>
            <div className="flex items-center gap-2">
              <button
              ...
              <span className="px-3 py-1 text-sm">
                Page {currentPage} of {Math.ceil(verifications.total / (verifications.limit || 10))}
              </span>
              <button
                onClick={() => setCurrentPage(prev => Math.min(Math.ceil(verifications.total / (verifications.limit || 10)), prev + 1))}
                disabled={currentPage === Math.ceil(verifications.total / (verifications.limit || 10))}
                className="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Next
              </button>
